...
# 编译完成后，可执行文件在 llama.cpp 目录下：
# - llama-cli (推理工具)
# - quantize (量化工具)
# - convert-hf-to-gguf.py (转换脚本)
```

**验证编译：**
```bash
# 检查可执行文件
ls -lh llama-cli quantize
...
**注意：**
- 如果脚本名称不同，可能是 `convert.py` 或 `convert_hf_to_gguf.py`
- 查看 llama.cpp 目录下的 Python 脚本确认正确的名称
...
创建测试脚本 `stage2_quantize/test_quantized_model.py` 进行批量评估。
